...
<?php
	...
	if (!empty($_POST))
	{
		...
		$query = "INSERT into boleto values (boleto_sequence.nextval, '$zona', '$fecha', '$idPartido', '$precio', '$idEstadio' , '$idCliente', '$idPago')";
...
<script>
		...
		function validar()
		{
          ...
          function repage(){
               window.open("boletoActualizar.php");
          }
        ...
        <div class="row justify-content-center">
            <div class="container">
                <div class="form-box-signin">
                    ...
                    <h2 class="mt-0">BOLETO</h2>
                    <hr class="divider my-5" />
                    </div>
                    <!-- Form SIGNIN-->
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" id="registrar" class="input-group">
                        <input type="text" name="zona" id="zona" class="input-field" placeholder="Zona">
                        <input type="text" name="fecha" id="fecha" class="input-field" placeholder="Fecha">
                        <input type="number" name="idPartido" id="idPartido" class="input-field" placeholder="ID Partido" required pattern="[A-Za-z0-9]{1,30}">
                        <input type="text" name="precio" id="precio" class="input-field" placeholder="Precio $" required pattern="[A-Za-z0-9]{1,15}">
                        <input type="number" name="idEstadio" id="idEstadio" class="input-field" placeholder="ID Estadio" required pattern="[A-Za-z0-9]{1,30}">
                        <input type="number" name="idCliente" id="idCliente" class="input-field" placeholder="ID Cliente" required pattern="[A-Za-z0-9]{1,30}">
                        <input type="number" name="idPago" id="idPago" class="input-field" placeholder="ID Pago" required pattern="[A-Za-z0-9]{1,30}">
                ...
                <div>
                    <input type="submit" class="submit-btn" value="Actualizar" onclick="repage()">
                </div>
            </div>
            <div class="curva">
